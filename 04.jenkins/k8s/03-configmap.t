...
data:
  jenkins.yaml: |
    jenkins:
      ...
      crumbIssuer:
        standard:
          excludeClientIPFromCrumb: true  # nginx reverse proxy 뒤에서 webhook crumb 검증 허용
      ...
      clouds:
        - kubernetes:
            ...
            serverUrl: ""
            namespace: "${NAMESPACE}"
            jenkinsUrl: "http://${USER_NAME}-jenkins:8080"
            jenkinsTunnel: "${USER_NAME}-jenkins-agent:50000"
            connectTimeout: 5
            readTimeout: 15
            ...
            templates:
              ...
              - name: "kubectl-agent"
                label: "kubectl"
                nodeUsageMode: EXCLUSIVE
                serviceAccount: "skala-admin-sa"      # kubectl이 클러스터 API에 접근하기 위해 jenkins SA 사용
                containers:
                  - name: "jnlp"
                    image: "jenkins/inbound-agent:latest-jdk21"
                    alwaysPullImage: false
                  ...
                  - name: "kubectl"
                    image: "alpine/k8s:1.29.2"    # alpine 기반 → /bin/sh 있음, kubectl 포함
                    command: "sleep"
                    args: "9999999"
                    workingDir: "/home/jenkins/agent"
                    resourceRequestCpu: "250m"
              ...
              - name: "kaniko-agent"
                ...
                containers:
                  ...
                  - name: "kaniko"
                    ...
                    envVars:
                      - envVar:
                          key: "DOCKER_CONFIG"              # kaniko가 인증 정보를 읽는 경로
                          value: "/kaniko/.docker"
                volumes:
                  - persistentVolumeClaim:
                      claimName: "${USER_NAME}-jenkins-pvc"
      ...
      system:
        domainCredentials:
          - credentials:
              - usernamePassword:
                  id: "skala-github-id"
                  description: "GitHub PAT (소스 클론 및 gitops 브랜치 푸시용)"
                  username: "${GIT_USERNAME}"
                  password: "${GIT_PASSWORD}"
                  scope: GLOBAL
              - usernamePassword:
                  id: "skala-image-registry-id"
                  description: "Harbor Registry 인증 (이미지 빌드/푸시용)"
    ...
    unclassified:
      location:
        url: "https://${USER_NAME}-jenkins.skala25a.project.skala-ai.com/"  # Jenkins Root URL (webhook 응답 URL 생성에 사용)
